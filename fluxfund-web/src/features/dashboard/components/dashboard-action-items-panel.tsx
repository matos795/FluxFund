      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`shrink-0 text-sm font-semibold ${
            variant === "danger" || isHighValue ? "text-foreground" : ""
          }`}
        >
...
function FundActionItem({ item }: { item: DashboardFundActionItem }) {
  ...
  return (
    <Link
      to="/reports/funds"
      className={`block border-l-4 px-3 py-2.5 transition hover:bg-muted/50 ${
    ...
    <section className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <DashboardActionListCard
          ...
          renderItem={(item) => (
            <TransactionActionItem item={item} variant="warning" />
          )}
          compact
        />
        ...
        <DashboardActionListCard
          ...
          getItemKey={(item) => item.transactionId}
          renderItem={(item) => (
            <TransactionActionItem item={item} variant="danger" />
          )}
          compact
        />
