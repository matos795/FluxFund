...
class TransactionAllocationRepositoryIntegrationTest {
        ...
        @Test
        void shouldIgnoreTechnicalMovementInFinancialRelationshipIncomeSources() {
                ...
                FinancialTransaction technicalTransaction = new FinancialTransaction();
                technicalTransaction.setOrganization(organization);
                technicalTransaction.setAccount(account);
                technicalTransaction.setSource(FinancialTransactionSource.OFX);
                technicalTransaction.setStatus(FinancialTransactionStatus.SETTLED);
                technicalTransaction.setType(FinancialTransactionType.INCOME);
                technicalTransaction.setSettlementDate(LocalDate.of(2026, 8, 11));
                technicalTransaction.setExpectedAmount(new BigDecimal("5000.00"));
                technicalTransaction.setSettledAmount(new BigDecimal("5000.00"));
                technicalTransaction.setInterestAmount(BigDecimal.ZERO);
                technicalTransaction.setDiscountAmount(BigDecimal.ZERO);
                technicalTransaction.setDescription("Movimento técnico legado");
                technicalTransaction.markAsTechnicalMovement(TechnicalMovementType.NUBANK_PIX_CREDIT_BRIDGE);

                entityManager.persist(technicalTransaction);

                TransactionAllocation technicalAllocation = new TransactionAllocation();
                technicalAllocation.setOrganization(organization);
                technicalAllocation.setFinancialTransaction(technicalTransaction);
                technicalAllocation.setFund(fund);
                technicalAllocation.setSourceParty(joao);
                technicalAllocation.setAmount(new BigDecimal("5000.00"));

                entityManager.persist(technicalAllocation);
